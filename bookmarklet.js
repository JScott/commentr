...
(function(){
  ...
  if (window.jQuery === undefined || window.jQuery.fn.jquery < v) {
    ...
    jqScript.src = '//ajax.googleapis.com/ajax/libs/jquery/' + v + '/jquery.min.js';
    ...
    jqScript.onload = jqScript.onreadystatechange = function(){
      if (!done && (!this.readyState || this.readyState == 'loaded' || this.readyState == 'complete')) {
        ...
        ioScript.onload = ioScript.onreadystatechange = function() {
          if (!done && (!this.readyState || this.readyState == 'loaded' || this.readyState == 'complete')) {
            window.console.log("SocketIO loaded");
            done = true;
            initMyBookmarklet();
          }
        }
  ...
  function initMyBookmarklet() {
    ...
    (window.myBookmarklet = function() {
          ...
          jQuery.getJSON(restUrl+host+'/'+encodeURIComponent(path), function(comments) {
            ...
            var barPositioning = 'z-index:1001;position:fixed;bottom:0;left:0;';
            ...
            */var powerBar = '<div class="'+uniqueClassAll+'" style="'+barFont+barStyle+barPositioning+'">'+author+'\'s Power Bar</div>';
            ...
            function highlightElement(selector, index) {
              ...
              if (offset) { /* Hidden elements aren't in the rendering tree */
                var highlightBoxPositioning = 'z-index:999;position:absolute;top:'+offset.top+'px;left:'+offset.left+'px;height:'+element.height()+'px;width:'+element.width()+'px;';
            ...
            var commentStyle = 'background:white;padding:5px;height:300px;width:350px;border:2px solid #595859;border-bottom:none;';
            var commentPositioning = 'position:absolute;z-index:1000;'/*z-index:1000;position:fixed;bottom:40px;right:0;';*/
            var commentZone = '<div class="'+uniqueClassAll+'" style="'+commentFont+commentStyle+commentPositioning+'"><div style="overflow-y:scroll;height:260px;" class="'+uniqueClassAll+' '+uniqueClassComments+'"></div></div>';
            var unselectButtonStyle= 'width:30px;height:30px;position:relative;bottom:280px;float:right;left:20px;';
            ...
            /*$('*:not(.'+uniqueClassAll+')').mouseover(function() {
              var mouseoverPositioning = 'z-index:999;position:relative;bottom:'+0+'px;left:'+0+'px;height:'+$(this).height()+'px;width:'+$(this).width()+'px;';
